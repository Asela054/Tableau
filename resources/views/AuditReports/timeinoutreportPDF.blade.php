...
@foreach ($pdfData as $data)
    <table class="report-table">
        <tr>
            <td colspan="7" style="text-align: center; font-size: 18px; font-weight: bold; padding: 10px; border-bottom: none; border-right:none; border-left:none;">
                Time In-Out Report
            </td>
        </tr>
        <tr class="titlerow">
            ...
            <td style="border-top: none; border-bottom: none; border-right:1px; border-left:none; text-align:left;"  colspan="4"><strong>Name:</strong> {{ $data['employee']->emp_fullname }} </td>
        </tr>
        <thead>
            <tr>
        ...
        <tbody>
            @foreach ($data['attendance'] as $attendance)
                ...
                <tr>
                    ...
                    <td>{{ $attendance['empno'] }}</td>
                    <td>{{ $attendance['Department'] }}</td>
                    <td colspan="4">{{ $attendance['holiday_name'] }}</td>
                </tr>
                @elseif ($attendance['type'] == '3')
                <tr>
                    <td>{{ $attendance['date'] }}</td>
                    <td>{{ $attendance['empno'] }}</td>
                    <td>{{ $attendance['Department'] }}</td>
