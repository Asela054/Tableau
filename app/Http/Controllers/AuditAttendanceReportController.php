...
class AuditAttendanceReportController extends Controller
{
    ...
    public function generatetimereport(Request $request) {
        ...
        foreach ($employees as $employee) {
            ...
            foreach ($period as $date) {
				...
				// echo $date->format('Y-m-d').'-->'.$date->format('N').'<br>';
				...
				if ($holidays) {
					...
					$approvedLeaves = DB::table('leaves')
						...
						->where(function($query) use ($currentDate) {
							$query->whereDate('leaves.leave_from', '<=', $currentDate)
								->whereDate('leaves.leave_to', '>=', $currentDate);
						})
						->select('leaves.emp_id', 'leave_types.leave_type as reason')
						->first();
					if ($approvedLeaves) {
						$attendanceData[] = [
							'date' => $currentDate,
							...
							'ot_hours' => 'Leave',
							'type' => '3',
							'holiday_name' => $approvedLeaves->reason
						];
					}
					else{
						if ($attendance) {
							...
							// If no attendance data, check if it's a Sunday
							$attendanceData[] = [
								'date' => $currentDate,
								'empno' => $employee->emp_id,
								'Department' => $employee->departmentname,
		...
		// return view('AuditReports.timeinoutreportPDF', compact('pdfData'));
	...
	public function auditgeneratetimereportexcel(Request $request)
	{
		...
		foreach ($employees as $employee) {
			// Add employee header (like in PDF)
			$sheet->setCellValue("A$row", 'Time In-Out Report');
			$sheet->mergeCells("A$row:H$row");
			$sheet->getStyle("A$row")->getFont()->setBold(true);
			$row++;

			$sheet->setCellValue("A$row", 'Emp No: ' . $employee->emp_id);
			$sheet->setCellValue("C$row", 'Name: ' . $employee->emp_fullname);
			...
			$sheet->setCellValue("D$row", 'In Time');
			$sheet->setCellValue("E$row", 'Out Time');
			$sheet->setCellValue("F$row", 'Duration');
			$sheet->setCellValue("G$row", 'OT Hours');
			...
			foreach ($period as $date) {
				...
				if ($holidays) {
					...
					$sheet->setCellValue("C$row", $employee->departmentname);
					$sheet->setCellValue("D$row", $holidays->holiday_name);
					...
					if ($approvedLeaves) {
						...
						$sheet->setCellValue("B$row", $employee->emp_id);
						$sheet->setCellValue("C$row", $employee->departmentname);
						$sheet->setCellValue("D$row", $approvedLeaves->reason);
						$sheet->mergeCells("D$row:H$row");
						$row++;
					} else {
						if ($attendance) {
							...
							$sheet->setCellValue("B$row", $employee->emp_id);
							$sheet->setCellValue("C$row", $employee->departmentname);
							$sheet->setCellValue("D$row", $attendance->in_time ? date('H:i:s', strtotime($attendance->in_time)) : '');
							$sheet->setCellValue("E$row", $attendance->out_time ? date('H:i:s', strtotime($attendance->out_time)) : '');
							$sheet->setCellValue("F$row", $attendance->duration);
							$sheet->setCellValue("G$row", $attendance->ot_hours);
							$row++;
						}
						else if($date->format('N') == 7){
							$sheet->setCellValue("A$row", $currentDate);
							$sheet->setCellValue("B$row", $employee->emp_id);
							$sheet->setCellValue("C$row", $employee->departmentname);
							$sheet->setCellValue("D$row", 'Sunday');
							$sheet->mergeCells("D$row:H$row");
							$row++;
						}
					}
			...
			// Add 3 empty rows between employees
			$row += 3;
		}

		$filename = 'Audit_Time_In_Out_Report_' . date('Y-m-d') . '.xlsx';
